...
public final class Mk3SwerveModuleHelper {
	...
	private static SteerControllerFactory<
					?, Falcon500SteerConfiguration<MagEncoderAbsoluteConfiguration>>
			getFalcon500SteerFactory(Mk3ModuleConfiguration configuration) {
		return new Falcon500SteerControllerFactoryBuilder()
				.withVoltageCompensation(configuration.getNominalVoltage())
				.withPidConstants(0.3, 0.0, 0.0)
				.withCurrentLimit(configuration.getSteerCurrentLimit())
				.build(new MagEncoderFactoryBuilder().withReadingUpdatePeriod(100).build());
	}

	/**
	 * Creates a Mk3 swerve module that uses Falcon 500s for driving and steering. Module
		...
		return new SwerveModuleFactory<>(
				...
				.create(
						...
						new Falcon500SteerConfiguration<>(
								steerMotorPort,
								new MagEncoderAbsoluteConfiguration(
