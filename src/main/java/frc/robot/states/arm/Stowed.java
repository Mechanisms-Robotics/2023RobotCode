...
public class Stowed extends ArmState {
	private static final double[][] ARM_POSITIONS = {
		{2750, -250, 0.05}, // Pivot, Extension | Cube
		{500, -750, 0.015} // Pivot, Extension | Cone
	};

	private static final double[][] GRIPPER_POSITIONS =
			new double[][] {
				{0, -17500, -17500}, // Open, Closed, Auto | Cube
				{0, -28000, -28000} // Open, Closed, Auto | Cone
			};
	...
	public Stowed(Arm arm, Gripper gripper, Supplier<Element> elementSupplier) {
		...
		m_holdPercent = ARM_POSITIONS[elementSupplier.get().index][2];
	...
	public void periodic() {
		if (m_elementSupplier.get() != m_prevElement) {
			...
			m_holdPercent = ARM_POSITIONS[m_elementSupplier.get().index][2];
