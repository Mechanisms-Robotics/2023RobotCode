...
public class Swerve extends SubsystemBase {
	...
	private static final double driveBaseWidth = 0.46355; // m
	private static final double driveBaseLength = 0.76835; // m
	...
	// Config injections into SDSlib to allow us to directly modify hardware objects (motors)
	private static final ModuleConfigCallback DRIVE_CONFIG =
			motor ->
					config ->
	...
	private static final ModuleConfigCallback STEER_CONFIG =
			motor ->
					config ->
							feedforwardConstants -> {
	...
	public void drive(double xVelocity, double yVelocity, Rotation2d rotation) {
		...
		desiredSpeeds =
				ChassisSpeeds.fromFieldRelativeSpeeds(xVelocity, yVelocity, 0.0, getHeading());
	}
	...
	/**
	 * Get positions of all the current modules.
	 *
	 * @return A list of SwerveModulePositions
	 */
	public SwerveModulePosition[] getModulePositions() {
		SwerveModulePosition[] positions = new SwerveModulePosition[4];
		positions[0] = flModule.getPosition();
		positions[1] = flModule.getPosition();
		positions[2] = flModule.getPosition();
	...
	public void zeroHeading() {
		...
		poseEstimator.resetPosition(
				getHeading(), getModulePositions(), new Pose2d(currentTranslation, zeroRotation));
	}
	...
	private Rotation2d getRawHeading() {
		// For the Pigeon 1 use getFusedHeading the Pigeon 2 uses getYaw. See the Pigeon 2 user
		// guide.
	...
	public double getVelocity() {
		return Math.sqrt(
				Math.pow(getSpeeds().vxMetersPerSecond, 2)
						+ Math.pow(getSpeeds().vyMetersPerSecond, 2));
	}
