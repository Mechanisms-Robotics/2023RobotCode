...
public interface SteerControllerFactory<Controller extends SteerController, SC> {
	...
	default Controller create(SC steerConfiguration, MechanicalConfiguration mechConfiguration) {
		return create(steerConfiguration, "", mechConfiguration);
	}

	Controller create(
			SC steerConfiguration, String canbus, MechanicalConfiguration mechConfiguration);
}
