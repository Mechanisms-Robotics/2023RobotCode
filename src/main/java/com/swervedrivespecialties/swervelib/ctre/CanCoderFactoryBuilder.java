...
public class CanCoderFactoryBuilder {
	...
	public AbsoluteEncoderFactory<CanCoderAbsoluteConfiguration> build() {
		return configuration -> {
			...
			WPI_CANCoder encoder =
					new WPI_CANCoder(configuration.getId(), configuration.getCanbus());
			CtreUtils.checkCtreError(
					encoder.configAllSettings(config, 250), "Failed to configure CANCoder");

			CtreUtils.checkCtreError(
					encoder.setStatusFramePeriod(
							CANCoderStatusFrame.SensorData, periodMilliseconds, 250),
	...
	private static class EncoderImplementation implements AbsoluteEncoder {
		private final int ATTEMPTS =
				3; // TODO: Allow changing number of tries for getting correct position
		...
		public double getAbsoluteAngle() {
			double angle = Math.toRadians(encoder.getPosition()) + offset;
