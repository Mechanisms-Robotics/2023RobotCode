...
public final class Falcon500SteerControllerFactoryBuilder {
	...
	private ModuleConfigCallback configFunction;
	...
	public Falcon500SteerControllerFactoryBuilder withMotorConfigFunction(
			ModuleConfigCallback configFunction) {
	...
	public <T>
			SteerControllerFactory<ControllerImplementation, Falcon500SteerConfiguration<T>> build(
	...
	private class FactoryImplementation<T>
			implements SteerControllerFactory<
					ControllerImplementation, Falcon500SteerConfiguration<T>> {
			...
			if (hasMotionMagic()) {
				if (hasVoltageCompensation()) {
					motorConfiguration.slot0.kF =
							(1023.0 * sensorVelocityCoefficient / nominalVoltage)
									* velocityConstant;
				}
	...
	private static class ControllerImplementation implements SteerController {
		...
		public void setReferenceAngle(double referenceAngleRadians) {
			...
			// Sometimes (~5% of the time) when we initialize, the absolute encoder isn't fully set
			// up,
			// and we don't
			...
			if (motor.getSelectedSensorVelocity() * motorEncoderVelocityCoefficient
				...
				if (++resetIteration >= ENCODER_RESET_ITERATIONS) {
					...
					motor.setSelectedSensorPosition(
							absoluteAngle / motorEncoderPositionCoefficient);
