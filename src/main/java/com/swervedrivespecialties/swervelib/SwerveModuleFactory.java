...
public class SwerveModuleFactory<DC, SC> {
	...
	public SwerveModule create(
			DC driveConfiguration, String driveCanbus, SC steerConfiguration, String steerCanbus) {
		var driveController =
				driveControllerFactory.create(driveConfiguration, driveCanbus, mechConfiguration);
		var steerController =
				steerControllerFactory.create(steerConfiguration, steerCanbus, mechConfiguration);
	...
	public SwerveModule create(DC driveConfiguration, SC steerConfiguration) {
		var driveController = driveControllerFactory.create(driveConfiguration, mechConfiguration);
		var steerController = steerControllerFactory.create(steerConfiguration, mechConfiguration);
	...
	public SwerveModule create(
			...
			String steerCanbus) {
		var driveController =
				driveControllerFactory.create(
						container, driveConfiguration, driveCanbus, mechConfiguration);
		var steerContainer =
				steerControllerFactory.create(
						container, steerConfiguration, steerCanbus, mechConfiguration);

		return new ModuleImplementation(driveController, steerContainer);
	}

	public SwerveModule create(
			ShuffleboardLayout container, DC driveConfiguration, SC steerConfiguration) {
		var driveController =
				driveControllerFactory.create(container, driveConfiguration, mechConfiguration);
		var steerContainer =
				steerControllerFactory.create(container, steerConfiguration, mechConfiguration);
	...
	private static class ModuleImplementation implements SwerveModule {
		...
		private ModuleImplementation(
				DriveController driveController, SteerController steerController) {
