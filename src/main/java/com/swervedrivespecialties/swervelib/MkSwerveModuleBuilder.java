...
public class MkSwerveModuleBuilder {
	...
	private static SteerControllerFactory<?, SteerConfiguration<CanCoderAbsoluteConfiguration>>
			getFalcon500SteerFactory(MkModuleConfiguration configuration) {
		return new Falcon500SteerControllerFactoryBuilder()
				...
				.withMotionMagic(
						configuration.getSteerMMkV(),
						configuration.getSteerMMkA(),
	...
	private SteerControllerFactory<?, SteerConfiguration<CanCoderAbsoluteConfiguration>>
			steerFactory = null;
	...
	public MkSwerveModuleBuilder withDriveMotor(
			MotorType motorType, int motorPort, String motorCanbus) {
		switch (motorType) {
			case FALCON:
	...
	public MkSwerveModuleBuilder withSteerMotor(
			MotorType motorType, int motorPort, String motorCanbus) {
		switch (motorType) {
			case FALCON:
	...
	public SwerveModule build() {
		...
		SwerveModuleFactory<Integer, SteerConfiguration<CanCoderAbsoluteConfiguration>> factory =
				new SwerveModuleFactory<>(mechConfig, driveFactory, steerFactory);
