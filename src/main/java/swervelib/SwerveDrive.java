...
public class SwerveDrive {
	...
	/**
	 * Creates a new swerve drivebase subsystem. Robot is controlled via the {@link
	 * SwerveDrive#drive} method, or via the {@link SwerveDrive#setModuleStates} method. The {@link
	 * SwerveDrive#drive} method incorporates kinematics-- it takes a translation and rotation, as
	 * well as parameters for field-centric and closed-loop velocity control. {@link
	 * SwerveDrive#setModuleStates} takes a list of SwerveModuleStates and directly passes them to
	 * the modules. This subsystem also handles odometry.
	 *
	...
	public SwerveDrive(
		...
		swerveDrivePoseEstimator =
				new SwerveDrivePoseEstimator(
						...
						stateStdDevs,
						visionMeasurementStdDevs); // x,y,heading in radians; Vision measurement std
		// dev, higher=less weight
		...
		if (SwerveDriveTelemetry.verbosity.ordinal() >= TelemetryVerbosity.HIGH.ordinal()) {
			...
			SwerveDriveTelemetry.sizeFrontBack =
					Units.metersToInches(
							SwerveMath.getSwerveModule(swerveModules, true, false)
											.moduleLocation
											.getX()
									+ SwerveMath.getSwerveModule(swerveModules, false, false)
											.moduleLocation
											.getX());
			SwerveDriveTelemetry.sizeLeftRight =
					Units.metersToInches(
							SwerveMath.getSwerveModule(swerveModules, false, true)
											.moduleLocation
											.getY()
									+ SwerveMath.getSwerveModule(swerveModules, false, false)
											.moduleLocation
	...
	public void drive(
		...
		if (SwerveDriveTelemetry.verbosity.ordinal() >= TelemetryVerbosity.HIGH.ordinal()) {
			...
			SwerveDriveTelemetry.desiredChassisSpeeds[2] =
					Math.toDegrees(velocity.omegaRadiansPerSecond);
		}
	...
	public ChassisSpeeds getFieldVelocity() {
		...
		// angle
		// given as the robot angle reverses the direction of rotation, and the conversion is
		// reversed.
	...
	public Rotation2d getYaw() {
		...
		if (!SwerveDriveTelemetry.isSimulation) {
			...
			return Rotation2d.fromDegrees(
					swerveDriveConfiguration.invertedIMU ? 360 - ypr[0] : ypr[0]);
		} else {
	...
	public Rotation2d getPitch() {
		...
		if (!SwerveDriveTelemetry.isSimulation) {
			...
			imu.getYawPitchRoll(ypr);
			return Rotation2d.fromDegrees(
					swerveDriveConfiguration.invertedIMU ? 360 - ypr[1] : ypr[1]);
		} else {
	...
	public Rotation2d getRoll() {
		...
		if (!SwerveDriveTelemetry.isSimulation) {
			...
			imu.getYawPitchRoll(ypr);
			return Rotation2d.fromDegrees(
					swerveDriveConfiguration.invertedIMU ? 360 - ypr[2] : ypr[2]);
		} else {
		...
		for (SwerveModule swerveModule : swerveModules) {
			SwerveModuleState2 desiredState =
					new SwerveModuleState2(
							0, swerveModule.configuration.moduleLocation.getAngle(), 0);
			if (SwerveDriveTelemetry.verbosity.ordinal() >= TelemetryVerbosity.HIGH.ordinal()) {
	...
	public void updateOdometry() {
		...
		for (SwerveModule module : swerveModules) {
			...
			if (SwerveDriveTelemetry.verbosity == TelemetryVerbosity.HIGH) {
				SmartDashboard.putNumber(
						"Module" + module.moduleNumber + "Relative Encoder",
						module.getRelativePosition());
				SmartDashboard.putNumber(
						"Module" + module.moduleNumber + "Absolute Encoder",
